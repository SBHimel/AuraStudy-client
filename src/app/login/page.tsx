...
export default function SignInPage() {
  ...
  // 🎯 Single Click Student Demo Login
  const triggerStudentDemo = async () => {
    setEmail(STUDENT_DEMO.email);
    setPassword(STUDENT_DEMO.password);
    setDemoLoading(true);
    toast.success("Initializing Student Demo Mode...");
    await handleLogin(STUDENT_DEMO.email, STUDENT_DEMO.password);
  };
    ...
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center px-6 py-12 bg-slate-950 relative overflow-hidden">
      ...
      >
        <Surface className="p-8 rounded-3xl border border-white/[0.06] bg-slate-900/60 backdrop-blur-xl shadow-2xl space-y-6">
          ...
          {/* 🎯 Evaluator 1-Click Demo Button (Placed at top for easy access) */}
          <div className="p-1 rounded-2xl bg-gradient-to-r from-emerald-500/20 via-cyan-500/20 to-emerald-500/20">
            <button
              type="button"
              disabled={loading || demoLoading}
              onClick={triggerStudentDemo}
              className="w-full py-4 rounded-xl font-bold bg-slate-900 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/10 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-70 shadow-lg shadow-emerald-500/10"
            >
              {demoLoading ? (
              ...
              {demoLoading ? "Logging in..." : "1-Click Demo Login (Student)"}
            </button>
          </div>
          ...
          <Form onSubmit={onSubmit} className="space-y-4">
            <TextField isRequired name="email" value={email}>
            ...
            <TextField isRequired name="password" value={password}>
              <Label className="text-slate-300 font-semibold text-xs tracking-wider uppercase mb-1 flex items-center gap-1.5">
          ...
          <Button
            ...
            className="w-full py-6 rounded-xl font-bold bg-slate-800 text-white border border-slate-700 hover:bg-slate-700 transition-colors flex items-center justify-center gap-3 cursor-pointer mt-4"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v3.927h6.6v3.18c-.28 1.47-1.11 2.7-2.36 3.53v2.93h3.8c2.23-2.05 3.7-5.07 3.7-8.51z" />
              <path fill="#34A853" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.8-2.93c-1.05.7-2.4 1.13-4.13 1.13-3.18 0-5.88-2.15-6.84-5.04H1.27v3.02C3.25 21.2 7.37 24 12 24z" />
              <path fill="#FBBC05" d="M5.16 14.25c-.25-.7-.39-1.46-.39-2.25s.14-1.55.39-2.25V6.73H1.27C.46 8.32 0 10.1 0 12s.46 3.68 1.27 5.27l3.89-3.02z" />
              <path fill="#EA4335" d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.44-3.44C17.93 1.19 15.22 0 12 0 7.37 0 3.25 2.8 1.27 6.73l3.89 3.02c.96-2.89 3.66-5.04 6.84-5.04z" />
            </svg>
